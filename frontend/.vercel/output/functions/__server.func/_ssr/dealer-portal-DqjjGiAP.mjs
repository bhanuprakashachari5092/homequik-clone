...
import { d as CircleX, e as LogOut, f as Briefcase, g as Clock, C as CircleCheck, P as Phone, h as MapPin } from "../_libs/lucide-react.mjs";
...
function DealerPortal() {
  ...
  const handleLogin = (e) => {
    ...
    if (storedDealers) {
      ...
      const foundDealer = dealers.find((d) => d.id === dealerId && d.phone === phone);
      if (foundDealer) {
...
function DealerDashboard({
  ...
  reactExports.useEffect(() => {
    ...
    if (storedBookings) {
      ...
      const myBookings = allBookings.filter((b) => b.dealerId === currentDealer.id);
      setBookings(myBookings);
    }
  }, [currentDealer.id]);
  const updateBookingStatus = (bookingId, newStatus) => {
    const updatedMyBookings = bookings.map((b) => b.id === bookingId ? {
      ...b,
      status: newStatus
    } : b);
    ...
    if (storedBookings) {
      const allBookings = JSON.parse(storedBookings);
      const updatedAll = allBookings.map((b) => b.id === bookingId ? {
        ...b,
        status: newStatus
      } : b);
